# CLAUDE.md migrated from top-level during Phase B-3B

Migration note: original content is preserved for later owner curation. Legacy Claude/Codex/Agents references may remain here intentionally and are not active Hermes profile instructions.
...
## 환경
- 프로젝트: Prompt-Hermes-Engine (Discord + Hermes Agent Actions + Supabase/R2 + OpenAI Sites)
- 런타임: Node.js 20+ / TypeScript
- 패키지 매니저: npm
- 빌드/실행 명령어: `npm run dev`, `npm run check`, `npm run build`, `npm start`
- 검증 명령어: `npm run check`, `.claude/scripts/docs_ops.py verify-relates`
- 배포 방식: 자체 백엔드 서버 없음. Discord Webhook, OpenAI Agent Actions, Supabase, Cloudflare R2, OpenAI Sites 구성으로 운영
